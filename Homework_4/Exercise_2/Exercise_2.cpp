...
auto packBitPrintCompress(std::ofstream &output, const std::string &characters, const bool similar, const char c, const int i) {
...
void compress(std::istreambuf_iterator<unsigned char> input, std::ofstream &output) {
    std::string binary;
    bool similar = false;
    char c;
    while (input.read(&c, 1)) {
        if (binary.size() < 128) {
            if ((binary.back() == c && similar) || (binary.back() == c && binary.size() == 1)) {
                ...
                if (!similar) {
                    if (binary.back() == c && binary.size() > 1) {
                        if (binary.at(binary.size() - 2) == c) {
                            binary.pop_back();
                            binary.pop_back();
                            binary = packBitPrintCompress(output, binary, similar, c, 2);
                            similar = true;
                        }
                    }
                    binary.push_back(c);
                } else {
                    binary = packBitPrintCompress(output, binary, similar, c, 1);
                    similar = false;
                }
            }
        } else {
            if (binary.back() == c && !similar) {
                char nc;
                input.read(&nc, 1);
                if (nc == c) {
                    binary.pop_back();
    ...
    if (!binary.empty()) {
        packBitPrintCompress(output, binary, similar, '\0', 1);
    }
